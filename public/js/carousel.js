document.addEventListener('DOMContentLoaded', function () {
    fetch('/api/userinfo')
        ...
        .then(data => {
            const userInfoSpan = document.getElementById('auth-status');
            const logoutButton = document.getElementById('logout-button');
            userInfoSpan.textContent = `Bienvenido, ${data.nombre} ${data.apellidos}!`;
            logoutButton.style.display = 'inline-block'; // Muestra el botón de logout si el usuario está autenticado
        })
        .catch(error => {
            console.error('Error al obtener la información del usuario:', error);
            document.getElementById('auth-status').textContent = 'No autenticado';
            // Oculta el botón de logout si no hay autenticación
            document.getElementById('logout-button').style.display = 'none';
        });

    // Manejador de clic para el botón de logout
